...
parfor ii = 1:length(traindb.label), %
	...
	%% Update Sep 10, Remove segment that has length smaller than min length
	[~, segment_name] = fileparts(segment_path);
	pattern = '(?<video>\w+)\.\w+\.frame(?<start_f>\d+)_(?<end_f>\d+)';
	info = regexp(segment_name, pattern, 'names');
	if ~isempty(info),
		start_frame = str2num(info.start_f);
		end_frame = str2num(info.end_f);
		if (end_frame - start_frame) < min_length,
			warning('Segment too short [%d frames]\n', end_frame - start_frame);
			continue;
		end	
	end
